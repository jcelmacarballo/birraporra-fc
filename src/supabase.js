...
export const auth = {
  ...
  async signIn(email, password) {
    ...
    if (error) {
      ...
      if (msg.includes("Email not confirmed")) return "Email no confirmat. Mira el correu.";
      return msg;
    }
    return null;
  },

  // Registre nou usuari amb email + contrasenya + metadata (nom, emoji)
  async signUp(email, password, name, emoji) {
    const { data, error } = await supabase.auth.signUp({
      email, password,
      options: { data: { name, emoji } },
    });
    if (error) {
      const msg = error.message || "";
      if (msg.includes("already registered")) return "Aquest email ja està registrat";
      if (msg.includes("at least 6")) return "La contrasenya ha de tenir 6+ caràcters";
      if (msg.includes("valid email")) return "Email no vàlid";
      return msg;
    }
    return null;
  },
